...
이 디렉터리의 문서는 2026-08-29 에 노션 데이터베이스 "[ADR] 설계 기록" 에서 마이그레이션했다. 형식은 MADR 축약판(상태 표, 배경, 선택지, 결정, 결과)이며, 각 문서 상단의 `원본:` 링크가 노션 원문을 가리킨다.
...
상태 매핑: 노션 `승인` → 채택, `폐기` → 폐기, `보류` → 제안(보류). 날짜는 노션 생성 일시의 KST 기준이다.
...
| [0010](0010-defer-threshold-tuning.md) | 데이터 기반 임계치·윈도우 최적화를 보류한다 | 제안(보류) | 2026-04-14 |
...
| [0017](0017-phased-validation-and-live-api.md) | 단계별 검증 및 실전 API 확장 전략 | 제안(보류) | 2026-04-21 |
...
| [0027](0027-window-and-weight-parameters.md) | 하이라이트 탐지 윈도우·가중치 파라미터 최적화 | 제안(보류) | 2026-04-27 |
...
| [0041](0041-virtual-viewer-and-avatar-state.md) | 가상 시청자 수 시뮬레이션 및 아바타 상태 동기화 | 제안(보류) | 2026-05-03 |
...
- 0015 는 노션에 배경 한 줄만 있고 결정·이유가 비어 있어 "(노션 원문 없음)" 으로 표시했다. 0024, 0037 은 배경·이유 항목이 비어 있어 결정 내용만 옮겼다.
- 0052 는 노션 원문이 결정 형식이 아닌 트러블슈팅 점검 목록이라, 절차를 결정 항목에 그대로 정리했다.
- 노션 원문의 개인 식별 정보(공용 계정 사용자 ID, 팀원 이름)는 `{user_id}` 등 자리 표시자로 바꾸거나 제거했다.
